...
void Analyzer::Plot(){
    ...
    TH1F* Hist3a = new TH1F("Hist3a", "Pt distribution", 100,  0, 1500);
    ...
    TH1F* Hist7 = new TH1F("Hist7", "ETA", 100,  -5, 5);
    TH1F* Hist8 = new TH1F("Hist8", "Separation between taus_vis", 100,  0, 5);
    int totalEntries = 0;
    int brh=0,brnh=0;
	 for (int i = 0; i < 30; i++) {
	 ...
	 for (Long64_t entry = 0; entry <tree->GetEntries() ; ++entry) {
        ...
        //CHECKING IF IT IS HADRONIC FOR THE FIRST PARTICLE
    	if (IsHadronicTau->at(0)==1){
            ...
            Hist3a->Fill(Particle1truth.Pt()/1000.); 
        ...
        if (IsHadronicTau->at(1)==1){
            ...
            Hist3a->Fill(Particle2truth.Pt()/1000.);
            brnh++;
			}
        totalEntries++;
        ...
        //dR
      		...
      		// Calculate deltaR using the TLorentzVector deltaR() method
    		double deltaR = Particle1.DeltaR(Particle2);
    
    		// Fill the histogram with deltaR
    		Hist8->Fill(deltaR);
...
cout<<brh+brnh<<" od   "<<totalEntries*2<<endl;	 
    ...
    legend->AddEntry(Hist3, "Pt of hadronic tau_truth", "f");  
    legend->AddEntry(Hist, "Pt of hadronic tau_vis", "f");
    ...
    canvas->cd(2);
    Hist3a->GetXaxis()->SetTitle("Pt [Gev]");
    Hist3a->GetYaxis()->SetTitle("Entries");
    Hist3a->GetXaxis()->SetRangeUser(0,800);
    Hist3a->GetYaxis()->SetRangeUser(0,16000);
    Hist2->SetLineColor(kAzure+2);
    Hist3a->SetLineColor(kPink+2);
    Hist3a->Draw("HIST");
	Hist2->Draw("HIST SAME");
    TLegend* legend1 = new TLegend(0.7, 0.7, 0.9, 0.9);  
    legend1->AddEntry(Hist3a, "Pt of non-hadronic tau_truth", "l");  
    legend1->AddEntry(Hist2, "Pt of non-hadronic tau_vis", "l");
	...
	legend1->Draw();
*/    
    //Hist8->GetXaxis()->SetTitle("dR");
    //Hist8->GetYaxis()->SetTitle("Entries");
    //Hist8->Draw("HIST");
	canvas->SaveAs("ptvis_vs_pttruthS.png"); 
